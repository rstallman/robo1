...
AvlTree.prototype.get = function (key) {
    ...
    var res = this._get(key, this._root);
...
AvlTree.prototype.localGetHeight = function() {
    return this.localHeight(this._root);
};

AvlTree.prototype.localHeight = function (n) {
    if (n === null) {
        return -1;
    }
    return 1 + Math.max(this.localHeight(n.left), this.localHeight(n.right));
};


// inorder travesal of the tree
AvlTree.prototype.print = function() {
    this.printAt(this._root);
};
...
AvlTree.prototype.asSortedArray = function() {
    return this._asSortedArray(this._root);
};
AvlTree.prototype._asSortedArray = function(root) {
    if(root === null) return [];
    var leftArray  = this._asSortedArray(root.left);
    var rightArray = this._asSortedArray(root.right);
    
    return leftArray.concat([[root.key, root.value]]).concat(rightArray);
};
