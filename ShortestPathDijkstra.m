...
while true
    ...
    [min_dist,idx] = min(distances);  % idx = extract min, run time will be O(n^2)
    
    if ((idx == dest) || isinf(min_dist))
       break;
    end
      
    i = find(map(idx,:)); % find edge index which incident of node idx
    ...
    for ii = i
        if distances(ii) > map(idx, ii) + min_dist;
            distances(ii) = map(idx, ii) + min_dist;
            parent(ii) = idx;
        end
    end
   
    % set distance to nan, inoder to prevent next time will extract min. not good, but works!!!!
    distances(idx) = nan;   
